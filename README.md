# Web Development Resources

Bu repository, **HTML, CSS, JavaScript, jQuery ve Bootstrap** başta olmak üzere web geliştirme sürecinde kullanılabilecek bilgi, örnek, çalışma ve hazır yapıları içermektedir.

## 📁 Klasörler

### 📄 Html

HTML ile ilgili temel ve ileri düzey bilgileri içerir.
Kodların açıklamaları ve dosyalarda kullanılan HTML yapıları **`Ozet.html`** içerisinde yer almaktadır.


### 🎨 Css

CSS ile ilgili kapsamlı ve ayrıntılı bilgiler bulunmaktadır.
Her dosyanın en alt kısmında ilgili konuya ait detaylı özet bulunmaktadır.
Kodların genel açıklamaları **`Ozet.html`** içerisinde yer almaktadır.
Günlük ve profesyonel web projelerinde kullanılabilecek CSS özelliklerinin büyük bir bölümünü içermektedir.


# SCSS (Sassy CSS)

SCSS, standart CSS'in yeteneklerini değişkenler, iç içe yazım (nesting), mixin'ler ve fonksiyonlarla artıran bir ön işleyicidir (preprocessor).

## 🚀 Kurulum ve Derleme (VS Code)

SCSS dosyaları tarayıcılar tarafından doğrudan okunamaz; `.css` dosyasına derlenmesi gerekir.

1. **Eklenti Kurulumu:** VS Code içinden **Live Sass Compiler** eklentisini yükleyin.
2. **Çalıştırma:** Oluşturduğunuz scss dosyasına gelin Alt barda(En altta) bulunan **`Watch Sass`** butonuna tıklayın. Göremiyorsanız: view → appearance → Status Bar.
3. **HTML Bağlantısı:** HTML dosyanıza oluşan `.css` dosyasını bağlayın: <link rel="stylesheet" href="style.css">


### ⚡ JavaScript

JavaScript'in temel yapılarının yanı sıra modern JavaScript özelliklerine de yer verilmiştir.
Başlamadan önce **`TemelBilgiler.html`** dosyasına bakınız.


### 🟢 JQuery

jQuery ile ilgili temel bilgiler, kullanım yapıları ve örnekler bulunmaktadır.
API Request gibi daha ileri düzey kod örneklerine de yer verilmiştir.
Başlamadan önce **`TemelBilgiler.html`** dosyasına bakınız.


### 🅱️ Bootstrap

Bootstrap'in temel yapıları ve responsive tasarım özellikleri bulunmaktadır.
Dosyaların içerisindeki kodların açıklamaları, ilgili kodların en altında yer almaktadır.


### 🛠️ Calisma

HTML, CSS, JavaScript ve diğer web teknolojileri kullanılarak hazırlanmış çalışmalar ve uygulamalar bulunmaktadır.


### 🔗 CDN

Projelerde kullanılabilecek hazır **CDN bağlantıları** ve kaynakları bulunmaktadır.


### 📚 CdnCalisma

CDN klasöründe bulunan kaynakların nasıl kullanılabileceği ve bu kaynaklarla ilgili bilgiler yer almaktadır.


### 🧩 HazirSablonlar

Web projelerinde kullanılabilecek **modern, kısa ve pratik hazır kod yapıları** bulunmaktadır.


---

## 🎯 Amaç

Bu repository'nin amacı, web geliştirme sürecinde ihtiyaç duyulan bilgileri, kod yapılarını, örnekleri ve hazır çözümleri **tek bir yerde toplamak** ve gerektiğinde hızlıca başvurulabilecek kapsamlı bir kaynak oluşturmaktır.

---

**Veysel KUŞ**
